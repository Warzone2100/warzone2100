...
function camGetArtifacts()
{
	...
	for (const alabel in __camArtifacts)
	{
		...
		const libLabel = __camGetArtifactLabel(alabel);
...
function __camPickupArtifact(artifact)
{
	...
	// FIXME: O(n) lookup here
	const alabel = __camGetArtifactKey(getLabel(artifact));
	const ai = __camArtifacts[alabel];
	...
	++__camNumArtifacts;
	const callback = __camGlobalContext()["camArtifactPickup_" + alabel];
	if (camDef(callback))
	{
		callback();
	}
...
function __camLetMeWinArtifacts()
{
	for (const alabel in __camArtifacts)
	{
		...
		if (ai.placed)
		{
			const label = __camGetArtifactLabel(alabel);
			const artifact = getObject(label);
