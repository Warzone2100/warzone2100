...
MORE_OIL_RANGE = function () return (10 * TILE) end
...
repeatingEvent(checkResearch, 40.0)
		...
		if structure ~= nil then
			if not friendlyPlayer(structure.player) and droid.health == 100 then
				...
				if found then
					buildX = droid.x
					buildY = droid.y
					found, buildX, buildY = pickStructLocationB(defStructs[count], buildX, buildY, me, -1)
					if found and droidCanReach(droid, buildX, buildY) then
	...
	while count < numFundamental do
		...
		if structure == nil then
			if isStructureAvailable(structs[count], me) then
				buildX = baseX
				buildY = baseY
				boolResult, buildX, buildY = pickStructLocationB(structs[count], buildX, buildY, me, 0)
				if boolResult == true then
					...
					while droid ~= nil do
						if (droid.order == DORDER_NONE or droid.order == DORDER_RTB) and droidCanReach(droid, buildX, buildY) then
							orderDroidStatsLoc(droid, DORDER_BUILD, structs[count], buildX, buildY)
...
function buildDerrick()
	...
	while droid ~= nil and not foundOne do
		if droid.order == DORDER_NONE or droid.order == DORDER_RTB then
			foundOne = true
		else
			droid = iterateGroup(buildGroup)
		end
	...
	if droid ~= nil then
		...
		_oil = getFeatureB(me)
		while _oil ~= nil and not foundOne do
			_newDist = distBetweenTwoPoints(droid.x, droid.y, _oil.x, _oil.y)
			_same = false
			
			-- this one is closer
			if _newDist < _bestDist and droidCanReach(droid, _oil.x, _oil.y) then
				if not threatInRange(me, _oil.x, _oil.y, OIL_THREAT_RANGE(), false) then
					initIterateGroup(buildGroup)
					_search = iterateGroup(buildGroup)
					while _search ~= nil and not foundOne do
						if _search.orderx == _oil.x and _search.ordery == _oil.y then
							_same = true
							foundOne = true
						end
						_search = iterateGroup(buildGroup)
	...
	repeatingEvent(buildDerrick, 8.0)
end
repeatingEvent(buildDerrick, 8.0)
			...
			if structure ~= nil then
				...
				if numFriendlyWeapStructsInRange(me, buildX, buildY, (3 * 128), false) < 2 then
					...
					if count2 > 0 then
						...
						-- pick a location
						boolResult, buildX, buildY = pickStructLocation(structChoice[count], buildX, buildY, me)
						
						_numBuilders = numBuildSameBuilding(NULLSTRUCTURESTAT, buildX, buildY)
						
						if (boolResult == true) and (_numBuilders < _maxBuilders) and droidCanReach(droid, buildX, buildY) then
...
repeatingEvent(buildOilDefenseOrRetreat, 12.0)
...
function buildPowerGenerators()
	...
	-- if we need powergen
	if (count2 * 4) < count then
		buildX = baseX
		buildY = baseY
		boolResult, buildX, buildY = pickStructLocation(powGen, buildX, buildY, me)
		if boolResult == true then
			initIterateGroup(buildGroup)
			droid = iterateGroup(buildGroup)
			while droid ~= nil do
...
repeatingEvent(buildPowerGenerators, 8.0)
...
function buildBase()
	...
	if idleGroup(buildGroup) >= (groupCountMembers(buildGroup) / 2) then
		count = 0
			...
			if structure == nil then
				if isStructureAvailable(structs[count], me) then
					...
					if boolResult == true then
						...
						initIterateGroup(buildGroup)
						droid = iterateGroup(buildGroup)
						while (droid ~= nil) and (_numBuilders < _maxBuilders) do
							if (droid.order == DORDER_NONE or droid.order == DORDER_RTB) and droidCanReach(droid, buildX, buildY) then
								orderDroidStatsLoc(droid, DORDER_BUILD, structs[count], buildX, buildY)
								_numBuilders = _numBuilders + 1
							end
...
function buildExpand()
	...
	-- loop round
	if extraStruct == numExtraStructs[curTech] then
		extraStruct = 0
	end
	
	if isStructureAvailable(extraStructs[curTech][extraStruct], me) then
		buildX = baseX
		buildY = baseY
		boolResult, buildX, buildY = pickStructLocationB(extraStructs[curTech][extraStruct], buildX, buildY, me, 0)
		...
		if boolResult == true then
			...
			droid = iterateGroup(buildGroup)
			while (droid ~= nil) and (_numBuilders < _maxBuilders) do
				if (droid.order ~= DORDER_BUILD) and (droid.order ~= DORDER_LINEBUILD) and droidCanReach(droid, buildX, buildY) then
					orderDroidStatsLoc(droid, DORDER_BUILD, extraStructs[curTech][extraStruct], buildX, buildY)
					_numBuilders = _numBuilders + 1
				end
				droid = iterateGroup(buildGroup)
			end
...
repeatingEvent(buildExpand, 60.0)
...
function upgradeStructures()
	...
	droid = iterateGroup(buildGroup)
	while droid ~= nil do
		if (droid.order ~= DORDER_BUILD) and (droid.order ~= DORDER_LINEBUILD) then
			boolResult = false
			
			if curTech == branchDefault then
				boolResult = upgradePowGen(droid, 2)
				
				-- factory
				if droid.order ~= DORDER_BUILD then
...
repeatingEvent(upgradeStructures, 36.0)

function upgradeFactory(_truck, _maxBuilders)
	local _factory = nil
	
	initEnumStruct(false, factory, me, me)
	_factory = enumStruct()
	while _factory ~= nil do
		if isStructureAvailable(facModule, me) and (skGetFactoryCapacity(_factory) < 2) and droidCanReach(_truck, _factory.x, _factory.y) then
			if (numBuildSameBuilding(facModule, _factory.x, _factory.y) + numBuildSameBuilding(vtolFactory, _factory.x, _factory.y)) < _maxBuilders then
				orderDroidStatsLoc(_truck, DORDER_BUILD, facModule, _factory.x, _factory.y)
				return true
			end
		end
		_factory = enumStruct()
...
function upgradeVtolFactory(_truck, _maxBuilders)
	local _factory = nil
	...
	while _factory ~= nil do
		if isStructureAvailable(facModule, me) and (skGetFactoryCapacity(_factory) < 2) and droidCanReach(_truck, _factory.x, _factory.y) then
			if (numBuildSameBuilding(facModule, _factory.x, _factory.y) + numBuildSameBuilding(factory, _factory.x, _factory.y)) < _maxBuilders then
				orderDroidStatsLoc(_truck, DORDER_BUILD, facModule, _factory.x, _factory.y)
				return true
			end
		end
...
function upgradeResearch(_truck, _maxBuilders)
	local _resFac = nil
	
	initEnumStruct(false, resLab, me, me)
	_resFac = enumStruct()
	while _resFac ~= nil do
		if isStructureAvailable(resModule, me) and (not testStructureModule(me, _resFac, 0)) and droidCanReach(_truck, _resFac.x, _resFac.y) then
			if (numBuildSameBuilding(resModule, _resFac.x, _resFac.y) + numBuildSameBuilding(resLab, _resFac.x, _resFac.y)) < _maxBuilders then
				orderDroidStatsLoc(_truck, DORDER_BUILD, resModule, _resFac.x, _resFac.y)
				return true
			end
		end
		_resFac = enumStruct()
...
function upgradePowGen(_truck, _maxBuilders)
	local _powGen = nil
	
	initEnumStruct(false, powGen, me, me)
	_powGen = enumStruct()
	while _powGen ~= nil do
		if isStructureAvailable(powModule, me) and (not testStructureModule(me, _powGen, 0)) and droidCanReach(_truck, _powGen.x, _powGen.y) then
			if (numBuildSameBuilding(powModule, _powGen.x, _powGen.y) + numBuildSameBuilding(powGen, _powGen.x, _powGen.y)) < _maxBuilders then
				orderDroidStatsLoc(_truck, DORDER_BUILD, powModule, _powGen.x, _powGen.y)
				return true
			end
		end
		_powGen = enumStruct()
...
function finishStructs()
	...
	while structure ~= nil do
		if not structureComplete(structure) then
			...
			droid = iterateGroup(buildGroup)
			while droid ~= nil do
				if droid.order ~= DORDER_BUILD and droid.order ~= DORDER_LINEBUILD and droidCanReach(droid, structure.x, structure.y) and distBetweenTwoPoints(droid.x, droid.y, structure.x, structure.y) < 20 * TILE then
...
repeatingEvent(finishStructs, 21.0)
...
repeatingEvent(newfortify, 100.0)
		...
		if (droid.droidType ~= DROID_TRANSPORTER) and (droid.droidType ~= DROID_COMMAND) then
			...
			if (droid.droidType == DROID_CONSTRUCT) or (droid.droidType == DROID_CYBORG_CONSTRUCT) then
				...
				if droid.droidType == DROID_CYBORG then
					...
					if groupCountMembers(scoutGroup) < numScouts[curTech] then
						...
						if groupCountMembers(attackGroup) < numAttackers[curTech] then
							...
							if groupCountMembers(defendGroup) < numDefenders[curTech] then
								...
								if groupCountMembers(scoutGroup) < maxScouts[curTech] then
									groupAddDroid(scoutGroup, droid)
								else
...
function droidBuilt(_droid, _structure)
	...
	if structure ~= nil then
		if droid.droidType == DROID_CONSTRUCT and fundamentalBeingBuilt ~= derrick then
			_fundie = getStructure(fundamentalBeingBuilt, me)
			if _fundie ~= nil then
				if not structureComplete(_fundie) then
...
function needTank()
	...
	if (groupCountMembers(scoutGroup) < maxScouts[curTech]) or (maxScouts[curTech] == UNLIMITED) then
		return true
	end
...
function factoryBuildDroid(_factory)
	...
	if structureIdle(_factory) then
		...
		while (_count2 < MAX_RANDOM_TEMPLATES) and (_count >= 0) do
			if skCanBuildTemplate(me, _factory, tmpl[curTech][_count]) then
...
function cyborgFactoryEvent()
	if not ((groupCountMembers(defendGroup) < maxCyborgs[curTech]) or (maxCyborgs[curTech] == UNLIMITED)) then
		return
	end
...
function cybFactorBuildCyborg(_factory)
	...
	if structureIdle(_factory) then
		if (groupCountMembers(defendGroup) < maxCyborgs[curTech]) and (getDroidCount(me) < MAX_DROIDS) then
			if random(5) == 1 then
...
function defendWatch(_structure, _baseobj)
	...
	if baseobj ~= nil then
		if not friendlyPlayer(baseobj.player) then
			if not isHumanPlayer(baseobj.player) then
		...
		if structureIdle(structure) then
			if structureComplete(structure) then
				...
				if not boolResult then
					if (maxIdleRes[curTech] == UNLIMITED) or (_numResearching < maxIdleRes[curTech]) then
...
function difficultyModifier()
	if not isHumanPlayer(me) then
		skDifficultyModifier(me)
	end
end
repeatingEvent(difficultyModifier, 60.0)
...
function vtolStructs()
	local _maxTrucks = 0
	local _numTrucks = 0
	local _maxBuildTrucks = 0
	local _minBuildTrucks = 0
	...
	_minBuildTrucks = 1
	_maxBuildTrucks = 3
	_maxTrucks = math.min(_maxBuildTrucks, math.max(_minBuildTrucks, idleGroup(buildGroup) / 2))
	
	-- if not enough
	if isStructureAvailable(vtolFactory, me) and _numVtolFacs < maxVtolFacs[curTech] then
		buildX = baseX
		buildY = baseY
		boolResult, buildX, buildY = pickStructLocation(vtolFactory, buildX, buildY, me)
		if boolResult == true then
			initIterateGroup(buildGroup)
			droid = iterateGroup(buildGroup)
			-- only send _maxTrucks to do the job
			_numTrucks = 0
			while (_numTrucks < _maxTrucks) and (droid ~= nil) do
				if (droid.order == DORDER_NONE or droid.order == DORDER_RTB) and droidCanReach(droid, buildX, buildY) then
					orderDroidStatsLoc(droid, DORDER_BUILD, vtolFactory, buildX, buildY)
					_numTrucks = _numTrucks + 1
				end
				droid = iterateGroup(buildGroup)
			end
		end
...
function buildRearmPads()
	...
	local _buildY = 0
	local _truck = nil
	local _buildOK = false
	
	if not isStructureAvailable(vtolPad, me) then
		return
	end
	...
	_buildOK, _buildX, _buildY = pickStructLocation(vtolPad, _buildX, _buildY, me)
	if _buildOK then
		initIterateGroup(buildGroup)
		_truck = iterateGroup(buildGroup)
		_buildOK = false
		while _truck ~= nil and _buildOK == false do
			if (_truck.order ~= DORDER_BUILD) and (_truck.order ~= DORDER_LINEBUILD) and droidCanReach(_truck, _buildX, _buildY) then
...
function vtolFactoryBuildVtol(_factory)
	local _numTemplates = 0
	local _bestTemplates = 0
	...
	if structureIdle(_factory) then
		...
		_bestTemplates = 0
		while (_bestTemplates < 3) and (_numTemplates >= 0) do
			if skCanBuildTemplate(me, _factory, vtols[_numTemplates]) then
				tmplChoice[_bestTemplates] = vtols[_numTemplates]
				_bestTemplates = _bestTemplates + 1
			end
			_numTemplates = _numTemplates - 1
		end
		
		if _bestTemplates > 0 then
			...
			if isVtol(objToDroid(baseobj)) then
				...
				while droid ~= nil do
					if (structure ~= nil) and (droid.order ~= DORDER_BUILD) then
						...
						if count2 ~= (-1) then
							boolResult, buildX, buildY = pickStructLocation(vtolDefStruct[count2], buildX, buildY, me)
...
function beaconEv(_player, _sender, _x, _y, _message)
	...
	ASSERT(sender >= 0 and sender < 8, "beaconEv: sender out of bounds: " .. sender, me)
...
function watchBaseThreat()
	...
	if baseInTrouble() then
		if not defendingOwnBase() then
			if _DEBUG then
				debugFile(me .. ") watchBaseThreat: base in trouble")
			end
...
function watchMenu()
	if _DEBUG then
		if vtolGrAttackObj[0] == nil then
			setDebugMenuEntry("0 - " .. groupCountMembers(vtolAttackGr[0]), 0)
		else
			setDebugMenuEntry("0 " .. groupCountMembers(vtolAttackGr[0]) .. " - " .. (vtolGrAttackObj[0].x / TILE) .. "-" .. (vtolGrAttackObj[0].y / TILE), 0)
		end
		
		if vtolGrAttackObj[1] == nil then
			setDebugMenuEntry("1 - " .. groupCountMembers(vtolAttackGr[1]), 1)
		else
			setDebugMenuEntry("1 " .. groupCountMembers(vtolAttackGr[1]) .. " - " .. (vtolGrAttackObj[1].x / TILE) .. "-" .. (vtolGrAttackObj[1].y / TILE), 1)
		end
		
		if vtolGrAttackObj[2] == nil then
			setDebugMenuEntry("2 - " .. groupCountMembers(vtolAttackGr[2]), 2)
		else
			setDebugMenuEntry("2 " .. groupCountMembers(vtolAttackGr[2]) .. " - " .. (vtolGrAttackObj[2].x / TILE) .. "-" .. (vtolGrAttackObj[2].y / TILE), 2)
		end
		
		if vtolGrAttackObj[3] == nil then
			setDebugMenuEntry("3 - " .. groupCountMembers(vtolAttackGr[3]), 3)
		else
			setDebugMenuEntry("3 " .. groupCountMembers(vtolAttackGr[3]) .. " - " .. (vtolGrAttackObj[3].x / TILE) .. "-" .. (vtolGrAttackObj[3].y / TILE), 3)
		end
		
		if vtolGrAttackObj[4] == nil then
			setDebugMenuEntry("4 - " .. groupCountMembers(vtolAttackGr[4]), 4)
		else
			setDebugMenuEntry("4 " .. groupCountMembers(vtolAttackGr[4]) .. " - " .. (vtolGrAttackObj[4].x / TILE) .. "-" .. (vtolGrAttackObj[4].y / TILE), 4)
		end
		
		if vtolGrAttackObj[5] == nil then
			setDebugMenuEntry("5 - " .. groupCountMembers(vtolAttackGr[5]), 5)
		else
			setDebugMenuEntry("5 " .. groupCountMembers(vtolAttackGr[5]) .. " - " .. (vtolGrAttackObj[5].x / TILE) .. "-" .. (vtolGrAttackObj[5].y / TILE), 5)
		end
	end
end
repeatingEvent(watchMenu, 1.0)
...
function structureDestroyed(_structure)
	...
	_count = 0
	while _count < numRebuildStat[curTech] do
		if structure.stat == rebuildStat[curTech][_count] then
...
function buildOnExactLocation(_truck, _x, _y, _stat)
	...
	__result, _newX, _newY = pickStructLocationB(_stat, _newX, _newY, me, -1)
	if __result then
		dbg("trying to rebuild on (" .. _newX .. "/" .. _newY .. ")", me)
		
		if (_x ~= _newX) or (_y ~= _newY) or not droidCanReach(_truck, _x, _y) then
			return
		end
...
if upgradeFactory == nil then upgradeFactory = function() print("stub: upgradeFactory"); return 0 end end
...
if upgradePowGen == nil then upgradePowGen = function() print("stub: upgradePowGen"); return 0 end end
...
if upgradeResearch == nil then upgradeResearch = function() print("stub: upgradeResearch"); return 0 end end
...
if upgradeVtolFactory == nil then upgradeVtolFactory = function() print("stub: upgradeVtolFactory"); return 0 end end
...
if setDebugMenuEntry == nil then setDebugMenuEntry = function() print("stub: setDebugMenuEntry"); return 0 end end
