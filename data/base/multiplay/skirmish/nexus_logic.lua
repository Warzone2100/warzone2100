	...
	while count < numFundamental do
		...
		if structure == nil then
			if isStructureAvailable(structs[count], me) then
				if grabTrucksAndBuild(12, structs[count], 0) then
					return
				end
			end
		end
			...
			if structure ~= nil then
				...
				if numFriendlyWeapStructsInRange(me, buildX, buildY, (3 * 128), false) < 2 then
					...
					if count2 > 0 then
						...
						-- pick a location
						boolResult, buildX, buildY = pickDroidStructLocation(droid, structChoice[count], buildX, buildY, me, -1)
...
function buildPowerGenerators()
	if not isStructureAvailable(powGen, me) then
		return
	end
	-- count = numderricks
	initEnumStruct(false, derrick, me, me)
	...
	if (count2 * 4) < count then
		...
		droid = iterateGroup(buildGroup)
		while droid ~= nil do
			if droid.order ~= DORDER_HELPBUILD and droid.order ~= DORDER_BUILD and droid.order ~= DORDER_LINEBUILD then
				buildX = baseX
				buildY = baseY
				__result, buildX, buildY = pickDroidStructLocation(droid, powGen, buildX, buildY, me, 1)
				if __result then
...
function buildBase()
	if idleGroup(buildGroup) >= (groupCountMembers(buildGroup) / 2) then
		...
		while count < numStructs do
			...
			if structure == nil then
				if isStructureAvailable(structs[count], me) then
					if grabTrucksAndBuild(10, structs[count], 0) then
						return
					end
				end
			end
...
function buildExpand()
	...
	if isStructureAvailable(extraStructs[curTech][extraStruct], me) then
		if not grabTrucksAndBuild(10, extraStructs[curTech][extraStruct], 0) then
