...
layout(std140) uniform cbuffer {
	...
	vec2 size;				// quad size in pixels
	vec2 gradientDir;		// (0,1)=vertical, (1,0)=horizontal, (0,0)=flat colorA
	vec4 colorA;			// fill at gradient start
	vec4 colorB;			// fill at gradient end
	vec4 borderColor;
	vec4 cornerRadii;		// px per corner: (topLeft, topRight, bottomLeft, bottomRight) - 0 = sharp
	vec4 clipRect;			// visible region in quad-local pixels (x0, y0, x1, y1) - x1 <= x0 = no clipping
	float gradientExponent;	// 1.0 = linear, 2.0 = (1-t)^2 falloff from colorA
	float borderWidth;		// pixels, 0 = none
	float edgeSoftness;		// alpha fade width at the SDF edge, pixels (1.0 = crisp AA)
};
