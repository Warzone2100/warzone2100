...
{
	vec3 N1 = texture(tex_nm, vec3(vec2(uv1.x, uv1.y+timeSec*0.04), 0.0), mipBias).xzy * vec3( 2.0, 2.0, 2.0) + vec3(-1.0, 0.0, -1.0);
	vec3 N2 = texture(tex_nm, vec3(vec2(uv2.x+timeSec*0.02, uv2.y), 1.0), mipBias).xzy * vec3(-2.0, 2.0,-2.0) + vec3( 1.0, -1.0, 1.0);
	vec3 N3 = texture(tex_nm, vec3(uv1.x+timeSec*0.05, uv1.y, 0.0), mipBias).xzy * 2.0 - 1.0;
	vec3 N4 = texture(tex_nm, vec3(uv2.x, uv2.y+timeSec*0.03, 1.0), mipBias).xzy * 2.0 - 1.0;
