...
function donateFromGroup(from, group)
{
	if (isDefined(group))
	{
		...
		switch (group)
		{
			...
			case "CYBORG": chosenGroup = enumGroup(attackGroup).filter((dr) => { return dr.droidType === DROID_CYBORG; }); break;
			case "VTOL": chosenGroup = enumGroup(vtolGroup); break;
			default: chosenGroup = enumGroup(attackGroup); break;
		}

		var droids = chosenGroup.filter((dr) => { return (dr.health > MIN_HEALTH); });
...
function initCobraGroups()
{
	...
	addDroidsToGroup(attackGroup, enumDroid(me, DROID_WEAPON).filter((obj) => { return !obj.isCB; }));
	addDroidsToGroup(attackGroup, enumDroid(me, DROID_CYBORG));
	addDroidsToGroup(vtolGroup, enumDroid(me).filter((obj) => { return isVTOL(obj); }));
	addDroidsToGroup(sensorGroup, enumDroid(me, DROID_SENSOR));
	addDroidsToGroup(artilleryGroup, enumDroid(me, DROID_WEAPON).filter((obj) => { return obj.isCB; }));
