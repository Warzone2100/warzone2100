...
function returnClosestEnemyFactory(enemyNumber)
{
	function uncached(enemyNumber)
	{
		...
		let facs = enumStruct(enemyNumber, FACTORY);
...
function checkAllForRepair()
{
	...
	const droids = enumGroup(attackGroup);
...
function findNearestEnemyDroid(enemy)
{
	function uncached(enemy)
	{
		...
		const badDroids = enumDroid(enemy);
		if (badDroids.length > 0)
		{
			let temp = badDroids.filter(function(dr) { return !isVTOL(dr); });
			if (temp.length === 0)
			{
				temp = badDroids;
			}
...
function findNearestEnemyStructure(enemy)
{
	function uncached(enemy)
	{
		...
		let s = enumStruct(enemy).filter(function(obj) { return (obj.stattype !== WALL); });
		if (s.length === 0)
		{
			s = enumStruct(enemy);
		}
...
function findNearestEnemyDerrick(enemy)
{
	function uncached(enemy)
	{
		...
		const derr = enumStruct(enemy, structures.derricks).sort(distanceToBase);
...
function artilleryTactics()
{
	...
	let sensors = enumGroup(sensorGroup).filter(function(dr) {
	...
	if (SENS_LEN + ARTI_LEN > 0)
	{
		...
		if (isDefined(obj))
		{
			const tempObj = getObject(obj.typeInfo, obj.playerInfo, obj.idInfo);
			...
			for (let i = 0; i < ARTI_LEN; ++i)
			{
					...
					(i < Math.floor(ARTI_LEN * subPersonalities[personality].beaconArtilleryPercentage)))
				{
					if (!beacon.wasVtol || (beacon.wasVtol && ARTILLERY_UNITS[i].weapons[0].canHitAir))
					{
						...
						const xRand = (random(100) < 50) ? random(15) : -random(15);
						const yRand = (random(100) < 50) ? random(15) : -random(15);
...
function recycleForHover()
{
	...
	const MIN_FACTORY = 1;
	const systems = enumDroid(me, DROID_CONSTRUCT).concat(enumDroid(me, DROID_SENSOR)).concat(enumDroid(me, DROID_REPAIR)).filter(function(dr) {
		return (dr.body !== "CyborgLightBody" && dr.propulsion !== "hover01");
	});
	const unfinished = unfinishedStructures();
	...
	if ((countStruct(structures.factory) > MIN_FACTORY) && componentAvailable("hover01"))
	{
		...
		if (forceHover)
		{
			const tanks = enumGroup(attackGroup).filter(function(dr) { return (dr.propulsion !== "hover01"); });
...
function repairDroidTactics()
{
	...
	const reps = enumGroup(repairGroup);
	const LEN = reps.length;

	if (LEN > 0)
	{
		let myDroids = enumGroup(attackGroup).filter(function(dr) {
...
function vtolTactics()
{
	...
	const MIN_VTOLS = 5;
	const vtols = enumGroup(vtolGroup).filter(function(dr) {
		return droidReady(dr.id);
	});
	...
	if (LEN >= MIN_VTOLS)
	{
		...
		if (isDefined(target))
		{
			for (let i = 0; i < LEN; ++i)
			{
				...
				{
					const pos = {x: vtols[i].x, y: vtols[i].y};
...
{
	const d = getObject(DROID, me, droidID);
	if (d === null || !isDefined(d.weapons[0]))
	{
		return;
	}

	const t = getObject(target.typeInfo, target.playerInfo, target.idInfo);
	if (t === null)
	{
		return;
	}

	const isScav = isDefined(scavengerPlayer) && t.player === scavengerPlayer;
...
{
	const area = cobraBaseArea();
	const enemyUnits = enumArea(area.x1, area.y1, area.x2, area.y2, ENEMIES, false).filter(function(obj) {
	...
	if (enemyUnits.length > 0)
	{
		...
		if (!beacon.disabled && (beacon.endTime < gameTime))
		{
			const mes = isVTOL(enemyUnits[0]) ? BEACON_VTOL_ALARM : undefined;
...
function donateSomePower()
{
	...
	{
		const ally = ALLY_PLAYERS[random(LEN)];
...
function baseShuffleDefensePattern()
{
	...
	const attackers = enumGroup(attackGroup).concat(enumGroup(artilleryGroup)).concat(enumGroup(vtolGroup));
...
function lowOilDefensePattern()
{
	...
	const derricks = enumStruct(me, structures.derrick);
	...
	for (let i = 0, len = attackers.length; i < len; ++i)
	{
		const derr = derricks[random(derricks.length)];
...
function retreatTactics()
{
	...
	const droids = enumGroup(retreatGroup);

	//Flee!
	for (let i = 0, len = droids.length; i < len; ++i)
	{
		const droid = droids[i];
		const friends = enumRange(droid.x, droid.y, SCAN_RADIUS, ALLIES, false).filter(function(obj) {
