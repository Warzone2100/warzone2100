...
function _weaponsGetGuns(num)
{
	...
	for (const t in guns_type)
	{
		...
		for (const g in guns_type[t])
		{
			...
			if (getResearch(guns_type[t][g][0]).done)
			{
				_weapon = guns_type[t][g][1];
				_dbg = research_name[guns_type[t][g][0]];
				break;
			}
		}
		...
		for (const r in guns_pts[t])
		{
			...
			if (getResearch(guns_pts[t][r]).done)
			{
				_points++;
			}
		}
	...
	if (_weapons.length > 0)
	{
		...
		for (const w in _weapons)
		{
			...
			for (let i = 0; i < _weapons[w][1]; ++i)
			{
				_out.push(_weapons[w][0]);
			}
		}
