...
function groupBuilders(droid)
{
	...
	if (droid)
	{
		...
		else if (policy['build'] !== 'rich' && earlyGame && distBetweenTwoPoints_p(base.x, base.y, droid.x, droid.y) < base_range && buildersMainLen < 2)
		{
			groupAdd(buildersMain, droid);
			debugMsg("buildersMain +1", 'group');
		}
		else if (policy['build'] !== 'rich' && earlyGame && distBetweenTwoPoints_p(base.x, base.y, droid.x, droid.y) > base_range)
		{
			groupAdd(buildersHunters, droid);
			debugMsg("...buildersHunters +1", 'group');
		}
		else if ((distBetweenTwoPoints_p(base.x, base.y, droid.x, droid.y) > base_range || (earlyGame && buildersHuntersLen < 2)) &&
			buildersMainLen >= 2 && policy['build'] !== 'rich')
		{
			groupAdd(buildersHunters, droid);
			debugMsg("....buildersHunters +1", 'group');
		}
		// Если основных строителей меньше минимальных, то добавляем новичка в группу основных строителей
		else if (buildersMainLen < minBuilders || factory_ready.length === 0)
		{
			groupAdd(buildersMain, droid);
			debugMsg("buildersMain +1", 'group');
		}
		// Если нет строителей-охотников, то добавляем к ним новичка
		else if (buildersHuntersLen < 1)
		{
			groupAdd(buildersHunters, droid);
			debugMsg(".buildersHunters +1", 'group');
		}
		// Держим такой баланс в группе: строителей на 1 больше чем строителей-охотников
		else if (buildersMainLen < (buildersHuntersLen + 1) && buildersMainLen < 5)
		{
			groupAdd(buildersMain, droid);
			debugMsg("buildersMain +1", 'group');
		}
...
function builderBuild(droid, structure, rotation, position)
{
	...
	switch (structure)
	{
		...
		case "A0PowerGenerator":
			...
			struct = power_gen;
			break;
		case "A0CommandCentre":
			struct = hq;
			break;
		case "A0Sat-linkCentre":
			struct = uplink_center;
			break;
		case "A0LasSatCommand":
			struct = lassat;
			break;
		case "A0ComDroidControl":
			struct = ccontrol;
			break;
	...
	if (struct.length > 0)
	{
		struct.forEach((obj) => {
			...
			if (obj.status === BEING_BUILT)
			{
				orderDroidObj_p(droid, DORDER_HELPBUILD, obj);
				stop = true;
				return true;
			}
		});
...
function buildersOrder(order, target)
{
	...
	if (!func_buildersOrder)
	{
		return false;
	}

	// function controller
	if (func_buildersOrder_trigger <= gameTime)
	{
		func_buildersOrder_trigger = gameTime + func_buildersOrder_timer;
	}
	else
	{
		return false;
	...
	*/
	...
	if (resource_extractor.length < maxExtractors || ally.length === 0)
	{
		...
		if (ally.length > 0)
		{
			builder_targets = filterNearAlly(builder_targets);
		}
	...
	{
		builder_targets = filterNearBase(builder_targets);
	}
	...
	if (defence.length > 0)
	{
		builder_targets = builder_targets.concat(oil_enemy);
	}
	...
	if (buildersMainLen > 0)
	{
		mainBuilders(rotation);
	}
	else
	{
		...
		{
			enumDroid(me, DROID_CONSTRUCT).forEach((e) => {
				groupBuilders(e);
			}); // Изыскиваем резервы
	...
	if (buildersHuntersLen > 0)
	{
		...
		*/
		...
		for (const h in hunters)
		{
			...
			{
				huntOnDuty = rigDefence(hunters[h]);
			}
			...
			if (huntOnDuty === false && !builderBusy(hunters[h]))
			{
				if (problemBuildings.length > 0)
				{
					...
					if (problemBuildings[0].status === BEING_BUILT)
					{
						orderDroidObj_p(hunters[h], DORDER_HELPBUILD, problemBuildings[0]);
						continue;
					}

					if (problemBuildings[0].health < 99)
					{
						orderDroidObj_p(hunters[h], DORDER_REPAIR, problemBuildings[0]);
						continue;
					}

					if (problemBuildings.length !== 1)
					{
				...
				if (distBetweenTwoPoints_p(hunters[h].x, hunters[h].y, base.x, base.y) > 10)
				{
					orderDroidLoc_p(hunters[h], DORDER_MOVE, base.x, base.y);
				}
...
function rigDefence(obj, nearbase)
{
	...
	if (!nearbase)
	{
		defQueue = filterNearBase(defQueue);
	}
	...
	if (nearbase && distBetweenTwoPoints_p(base.x, base.y, defQueue[0].x, defQueue[0].y) > base_range / 2)
	{
		return false;
	}
...
function defenceQueue()
{
	...
	if (myRigs.length >= 3 || policy['build'] === 'rich')
	{
		defQueue = myRigs.filter(
			(e) => {
				if (myDefence.length === 0) // Если защитных сооружений вообще нет, добавляем все координаты всех наших качалок
				{
					return true;
				}
				if (!getInfoNear(e.x, e.y, 'buildDef', 5, 30000, false).value) // Если не получается построить рядом защиту - запоминаем это на 5 минут и пропускаем
				{
					return false;
				}
				let defNum = 0;
				for (const i in myDefence)
				{
					...
					if (rage === EASY && defNum > 0)
					{
						return false;
					}
					if ((rage === MEDIUM || rage === HARD || berserk) && defNum >= 1) // если к качалке близко есть башня, пропускаем
					{
						return false;
					}
					if (rage === INSANE && defNum >= 2)
					{
						return false;
					}
				}
...
function oilHunt(obj, nearbase)
{
	...
	*/
	...
	if ((rage === HARD || rage === INSANE) && policy['build'] === 'rich')
	{
		builder_targets = sortByDistance(builder_targets, obj);
	}
	...
	for (const i in builder_targets)
	{
		if (getInfoNear(builder_targets[i].x, builder_targets[i].y, 'safe').value)
		{
			...
			{
				if (typeof builder_targets[i] === "undefined")
				{
					debugMsg("ERROR in oilHunt(): Выход за пределы массива, исправить!", 'error');
					break;
				}
				...
				if (builder_targets[i].type === FEATURE &&
					getInfoNear(builder_targets[i].x, builder_targets[i].y, 'actionTry', 0, 300000, false, false, true).value < feature_try)
				{
				...
				{
					if (builder_targets[i].status === BEING_BUILT)
					{
						orderDroidObj_p(obj, DORDER_HELPBUILD, builder_targets[i]);
					}
					else
					{
						orderDroidObj_p(obj, DORDER_REPAIR, builder_targets[i]);
					}

					builder_targets.splice(i, 1);
					return true;
				}
				else if (defence.length > 0 &&
					builder_targets[i].type === STRUCTURE &&
					builder_targets[i].stattype === RESOURCE_EXTRACTOR &&
					builder_targets[i].player !== me)
				{
					...
					if (pos &&
						!builderBusy(obj) &&
						(!getInfoNear(builder_targets[i].x, builder_targets[i].y, 'defended').value ||
						(playerPower(me) > 500 || berserk)))
					{
						orderDroidBuild_p(obj, DORDER_BUILD, toBuild, pos.x, pos.y, 0);
						// debugMsg("oilHunt(): Строим вышку у вражеского ресурса");
						return true;
	...
	if (typeof builder_targets === "undefined" ||
		builder_targets.length === 0 ||
		typeof builder_targets[0] === "undefined")
	{
		return false;
	}
	...
	if (nearbase)
	{
		if (distBetweenTwoPoints_p(base.x, base.y, builder_targets[0].x, builder_targets[0].y) > (base_range / 2))
		{
	...
	if (policy['build'] === 'rich')
	{
		if (distBetweenTwoPoints_p(base.x, base.y, builder_targets[0].x, builder_targets[0].y) > base_range)
		{
...
{
	// v2.7 test MOVE=BUSY
	// if (builder.order === DORDER_MOVE) return true;
	// if (builder.order === DORDER_MOVE && getDistance(builder) > 3) return true;
	return (builder.order === DORDER_BUILD ||
		builder.order === DORDER_HELPBUILD ||
		builder.order === DORDER_LINEBUILD ||
		builder.order === DORDER_DEMOLISH ||
		builder.order === 21); // DORDER_RECYCLE for 3.1.5 compatible
}
...
function builderRecycleWalls(builder)
{
	...
	{
		myWalls = myWalls.concat(filterOutBase(enumStruct(me, DEFENSE)));
	}
	...
	if (orderDroidObj_p(builder, DORDER_DEMOLISH, myWalls[0]))
	{
		return true;
	}
}
