...
function doResearch(){
	...
	labs = labs.filter((e) => {if(e.status == BUILT && structureIdle(e))return true;return false;});
	...
	var avail_research = enumResearch().filter((o) => {if(o.started)return false;return true;});
...
//	debugMsg('research_path.length:'+research_path.length, 'research');
	...
	if(research_path.length > 0 ){
		...
		prepare_research = research_path.filter((o) => {if(getResearch(o).started)return false; return true;});
...
//		to_research = to_research.filter((o) => {return researches.indexOf(o)!==-1});
...
//	if(findResearch(to_research[0]).filter((o) => {if(getResearch(o).started)return false; return true;}}).length === 0)
...
// 	if(countStruct('A0ResourceExtractor', me) < 8 && playerPower(me) < 1000 && enumStruct(me, RESEARCH_LAB).filter((e) => {if(!structureIdle(e)&&e.status==BUILT)return true;return false;}).length >= 3) return;
// 	if(countStruct('A0ResourceExtractor', me) < 5 && playerPower(me) < 500 && enumStruct(me, RESEARCH_LAB).filter((e) => {if(!structureIdle(e)&&e.status==BUILT)return true;return false;}).length >= 2) return;
// 	if(countStruct('A0ResourceExtractor', me) <= 3 && playerPower(me) < 300 && enumStruct(me, RESEARCH_LAB).filter((e) => {if(!structureIdle(e)&&e.status==BUILT)return true;return false;}).length >= 1) return;
