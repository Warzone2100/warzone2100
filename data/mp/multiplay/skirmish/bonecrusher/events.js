	...
	switch (structure.stattype)
	{
		case RESEARCH_LAB:
			...
			if ((rage !== EASY) && (gameTime < 300000) && (groupSize(buildersMain) > 0))
			{
				...
				{
					enumGroup(buildersMain).forEach((e, i) => {
						if (i > 0)
						{
...
function bc_eventDroidBuilt(droid, structure)
{
	...
	switch (structure.stattype)
	{
		...
		case CYBORG_FACTORY:
			...
			break;
		case VTOL_FACTORY:
			if (vtolToPlayer !== false)
			{
				donateObject(droid, vtolToPlayer);
			}
			else
			{
...
function bc_eventAttacked(victim, attacker)
{
	...
	if (isFixVTOL(attacker) && distBetweenTwoPoints_p(victim.x, victim.y, base.x, base.y) < base_range)
	{
	...
	if (victim.type === DROID && victim.droidType === DROID_WEAPON && !isFixVTOL(victim))
	{
		...
		{
			myDroids.forEach((o) => {
				if (Stats.Weapon[o.weapons[0].fullname].ImpactType !== "HEAT")
				{
					fleetDroid(o);
				}
				// groupAdd(droidsFleet, o);
...
function bc_eventChat(sender, to, message)
{
	...
	if ((sender !== me) &&
		allianceExistsBetween(me, sender) &&
		(message.substr(0, 3) === "bc "))
	{
		switch (message.substr(3))
		{
			...
			case "gmp":
				...
				{
					chat(sender, "My power is too low");
				}
				break;
			case "gtn":
				if (groupSize(buildersMain) === 0)
