...
function bc_eventResearched(research, structure, player) {
	...
	if(research.name == 'R-Wpn-MG4'){cyborgs = cyborgs.filter((e) => {if(e[2] == 'CyborgChaingun')return false;return true;});}
	if(research.name == 'R-Wpn-Flame2'){cyborgs = cyborgs.filter((e) => {if(e[2] == 'CyborgFlamer01')return false;return true;});}
...
function bc_eventPlayerLeft(player) {
	bc_ally = bc_ally.filter((p) => {if(p==player) return false; return true;});
	ally = ally.filter((p) => {if(p==player) return false; return true;});
	enemy = enemy.filter((p) => {if(p==player) return false; return true;});
...
//			partisanTrigger = gameTime + reactPartisanTimer;
	...
	if(((victim.type == DROID && victim.droidType == DROID_CONSTRUCT) || (victim.type == STRUCTURE)) && gameTime > eventsRun['targetRegular']){
		...
		if(distBetweenTwoPoints_p(lastImpact.x, lastImpact.y, base.x, base.y) < base_range){
			var warriors = enumRange(victim.x, victim.y, 20, ALLIES).filter((e) => {if(e.player == me && e.type == DROID && e.droidType == DROID_WEAPON)return true; return false;});
...
//			partisanTrigger = gameTime + reactPartisanTimer;
		...
		if(getWeaponInfo(victim.weapons[0].name).impactClass == "HEAT"){
			var enemies = enumRange(victim.x, victim.y, 3, ENEMIES).filter((e) => {
				return (e.type == DROID);
			});
			if(enemies.length > 0){
