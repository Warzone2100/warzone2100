...
_global.isHoverPropulsion = function(str) {
	return propulsionStats.some((val) => { return val.usage === PROPULSIONUSAGE.HOVER && val.stat === str; });
}

_global.isVtolPropulsion = function(str) {
	return propulsionStats.some((val) => { return val.usage === PROPULSIONUSAGE.VTOL && val.stat === str; });
}

_global.iHaveHover = function() {
	return propulsionStats.some((val) => { return val.usage === PROPULSIONUSAGE.HOVER && componentAvailable(val.stat);} );
}

_global.iHaveVtol = function() {
	return propulsionStats.some((val) => { return val.usage === PROPULSIONUSAGE.VTOL && componentAvailable(val.stat);} );
}
...
_global.enumStructList = function(list, player) {
	if (!defined(player))
		player = me;
	return list.reduce((summ, new_value) => { return summ.concat(enumStruct(player, new_value)); }, []);
}

_global.countStructList = function(list, player) {
	if (!defined(player))
		player = me;
	return list.reduce((summ, new_value) => { return summ + countStruct(new_value, player); }, 0);
}
...
_global.structListLimit = function(list) {
	return list.reduce((summ, val) => {return summ + getStructureLimit(val)}, 0)
}
...
_global.guessWeaponRole = function (name) {
	for (const stat in weaponStats) {
		if (
			...
			weaponStats[stat].templates.some((i) => {return i.weapons.indexOf(name) > -1;})
		)
...
function guessWeaponMicro(name) {
	function uncached() {
		for (const stat in weaponStats)
		{
			...
			if (weaponStats[stat].templates.some((i) => { return i.weapons.indexOf(name) > -1; }))
				return weaponStats[stat].micro;
		}
	}
...
function weaponPathIsAvailable(path, objectType, defrole) {
	switch(objectType) {
		case 0:
			return path.weapons.some((val) => { return componentAvailable(val.stat); })
		case 1:
		...
		case 2:
			return path.defenses.some((val) => { return val.defrole === defrole && isAvailable(val.stat); });
		case 3:
			return path.vtols.some((val) => { return componentAvailable(val.stat); });
		default: // research
			return true;
	}
}
...
function statsToResList(path) {
	return path.map((val) => { return val.res; });
}
...
_global.filterDataByFlag = function(data, attr_name, flag) {
		return data.filter((obj) => { return obj[attr_name] & flag; });
}
...
_global.getPropulsionStatsComponents = function(usage) {
	...
	return data.map((val) => { return val.stat; }).reverse()
}
