...
if [[ $DISTRO == "raspberrypios" || $DISTRO == "debian" ]]; then
  ...
  if [[ "${VERSION_PARTS[0]}" == "12" || "${VERSION_PARTS[0]}" == "13" ]]; then
    ...
    DEBIAN_FRONTEND=noninteractive apt -y install cmake git zip unzip gettext asciidoctor libphysfs-dev libpng-dev libopenal-dev libvorbis-dev libogg-dev libopus-dev libtheora-dev libxrandr-dev libfreetype-dev libfribidi-dev libharfbuzz-dev libcurl4-gnutls-dev gnutls-dev libsodium-dev libsqlite3-dev libprotobuf-dev protobuf-compiler libzip-dev libturbojpeg0-dev libzstd-dev
...
if [ "${DISTRO}" == "ubuntu" ]; then
  ...
  if [ "${VERSION_PARTS[0]}" -eq "18" ]; then
    ...
    DEBIAN_FRONTEND=noninteractive apt-get -y install cmake git zip unzip gettext asciidoctor libphysfs-dev libpng-dev libopenal-dev libvorbis-dev libogg-dev libopus-dev libtheora-dev libxrandr-dev libfreetype6-dev libfribidi-dev libharfbuzz-dev libcurl4-gnutls-dev gnutls-dev libsodium-dev libsqlite3-dev libprotobuf-dev protobuf-compiler libzip-dev libturbojpeg0-dev libzstd-dev
    echo "WARN: You will need to compile and install SDL3 from source!"
    echo "WARN: Ubuntu 18.04 ships zstd 1.3.3, which is below the 1.4.0 minimum required by -DWZ_ENABLE_ZSTD_COMPRESSION_ADAPTER=ON. The zstd compression adapter will not be available on this distro version."
...
if [ "${DISTRO}" == "fedora" ]; then
  ...
  dnf -y install cmake git $FEDORA_7ZIP_PACKAGE gettext rubygem-asciidoctor SDL3-devel physfs-devel libpng-devel openal-soft-devel libvorbis-devel libogg-devel opus-devel libtheora-devel freetype-devel fribidi-devel harfbuzz-devel libcurl-devel libsodium-devel sqlite-devel protobuf-devel libzip-devel libjpeg-turbo-devel libzstd-devel
  # Required because of broken CMake config files installed by libzip-dev:
...
if [ "${DISTRO}" == "alpine" ]; then
  ...
  apk add --no-cache cmake git p7zip gettext asciidoctor sdl3-dev physfs-dev libpng-dev openal-soft-dev libvorbis-dev libogg-dev opus-dev libtheora-dev freetype-dev fribidi-dev harfbuzz-dev curl-dev libsodium-dev sqlite-dev protobuf-dev libzip-dev libjpeg-turbo-dev zstd-dev
fi
...
if [ "${DISTRO}" == "archlinux" ]; then
  ...
  pacman -S --noconfirm cmake git p7zip gettext asciidoctor sdl3 physfs libpng openal libvorbis libogg opus libtheora xorg-xrandr freetype2 fribidi harfbuzz curl libsodium sqlite protobuf libzip libjpeg-turbo zstd
fi
