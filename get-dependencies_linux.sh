...
if [ "${DISTRO}" == "ubuntu" ]; then
  ...
  if [ "${VERSION_PARTS[0]}" -eq "18" ]; then
    ...
    DEBIAN_FRONTEND=noninteractive apt-get -y install cmake git zip unzip gettext asciidoctor libsdl2-dev libphysfs-dev libpng-dev libopenal-dev libvorbis-dev libtheora-dev libxrandr-dev libfreetype6-dev libharfbuzz-dev libcurl4-gnutls-dev gnutls-dev libsodium-dev qtscript5-dev qt5-default
  elif [ "${VERSION_PARTS[0]}" -ge "20" ]; then
    echo "Installing build-dependencies for Ubuntu 20.x+"
    DEBIAN_FRONTEND=noninteractive apt-get -y install cmake git zip unzip gettext asciidoctor libsdl2-dev libphysfs-dev libpng-dev libopenal-dev libvorbis-dev libtheora-dev libxrandr-dev libfreetype-dev libharfbuzz-dev libcurl4-gnutls-dev gnutls-dev libsodium-dev qtscript5-dev qt5-default
  else
...
if [ "${DISTRO}" == "fedora" ]; then
  ...
  dnf -y install cmake git p7zip gettext rubygem-asciidoctor SDL2-devel physfs-devel libpng-devel openal-soft-devel libvorbis-devel libogg-devel libtheora-devel freetype-devel harfbuzz-devel libcurl-devel openssl-devel libsodium-devel qt5-qtbase-devel qt5-qtscript-devel
...
if [ "${DISTRO}" == "alpine" ]; then
  ...
  apk add --no-cache cmake git p7zip gettext asciidoctor sdl2-dev physfs-dev libpng-dev openal-soft-dev libvorbis-dev libogg-dev libtheora-dev freetype-dev harfbuzz-dev curl-dev libsodium-dev qt5-qtbase-dev qt5-qtscript-dev
fi
...
if [ "${DISTRO}" == "archlinux" ]; then
  ...
  pacman -S --noconfirm cmake git p7zip gettext asciidoctor sdl2 physfs libpng openal libvorbis libogg libtheora xorg-xrandr freetype2 harfbuzz curl libsodium qt5-base qt5-script
fi
