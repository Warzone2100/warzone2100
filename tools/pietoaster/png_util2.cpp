...
bool iV_loadImage_PNG(const char *fileName, iV_Image *image)
{
	...
	if (fileHandle == NULL)
	{
		fprintf(stderr, "pie_PNGLoadFile: PHYSFS_openRead(%s) failed with error: %s\n", fileName, PHYSFS_getLastError());
	...
	{
		fprintf(stderr, "pie_PNGLoadFile: PHYSFS_read(%s) failed with error: %s\n", fileName, PHYSFS_getLastError());
		PNGReadCleanup(&info_ptr, &png_ptr, fileHandle);
		return false;
	}
	...
	{
		fprintf(stderr, "pie_PNGLoadMem: Did not recognize PNG header in %s", fileName);
		PNGReadCleanup(&info_ptr, &png_ptr, fileHandle);
		return false;
	}
	...
	if (png_ptr == NULL) {
		fprintf(stderr, "pie_PNGLoadMem: Unable to create png struct");
		PNGReadCleanup(&info_ptr, &png_ptr, fileHandle);
		return false;
	}
	...
	if (info_ptr == NULL) {
		fprintf(stderr, "pie_PNGLoadMem: Unable to create png info struct");
		PNGReadCleanup(&info_ptr, &png_ptr, fileHandle);
		return false;
	}
	...
	if (setjmp(png_jmpbuf(png_ptr))) {
		fprintf(stderr, "pie_PNGLoadMem: Error decoding PNG data in %s", fileName);
		PNGReadCleanup(&info_ptr, &png_ptr, fileHandle);
		return false;
	}
...
// 	png_set_gray_1_2_4_to_8(png_ptr);
	...
	image->bmp = (unsigned char*)malloc(info_ptr->height * info_ptr->rowbytes);
...
void iV_saveImage_PNG(const char *fileName, const iV_Image *image)
{
	...
	if (fileHandle == NULL)
	{
		fprintf(stderr, "pie_PNGSaveFile: PHYSFS_openWrite failed (while openening file %s) with error: %s\n", fileName, PHYSFS_getLastError());
		return;
	}
	...
	if (png_ptr == NULL)
	{
		fprintf(stderr, "pie_PNGSaveFile: Unable to create png struct\n");
	...
	if (info_ptr == NULL)
	{
		fprintf(stderr,  "pie_PNGSaveFile: Unable to create png info struct\n");
		return PNGWriteCleanup(&info_ptr, &png_ptr, fileHandle);
	}
	...
	if (setjmp(png_jmpbuf(png_ptr)))
	{
		fprintf(stderr, "pie_PNGSaveFile: Error encoding PNG data\n");
	}
	else
	{
		...
		scanlines = (const unsigned char**)malloc(sizeof(const unsigned char*) * image->height);
		if (scanlines == NULL)
		{
			fprintf(stderr, "pie_PNGSaveFile: Couldn't allocate memory\n");
			return PNGWriteCleanup(&info_ptr, &png_ptr, fileHandle);
		}
