...
static int parsePathfindingBackendArg(const char *arg)
{
	...
	if ((mask & ~validMask) != 0)
	{
		qFatal("Unknown pathfinding feature bits in \"%s\": valid features are 1 directional bias, 2 corridor lanes, 4 flow cost, 8 soft collision, 16 crowd mass, 32 bend hold, 64 bend hand, 128 wide lanes, 256 hand joint, 512 wide queue", arg);
	}
