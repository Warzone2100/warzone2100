...
Vector2i luaWZ_checkWorldCoords(lua_State *L, int param)
{
	...
	ASSERT_OR_RETURN(Vector2i(0, 0), worldOnMap(x, y), "Off map (%d, %d)", x, y);
	return Vector2i(x, y);
}
...
void luaWZObj_pushweapon(lua_State *L, WEAPON* weapon)
{
	...
	luaWZ_setintfield(L, "pitch", weapon->rot.pitch);
	luaWZ_setintfield(L, "rotation", weapon->rot.direction);
}
...
void luaWZObj_pushtemplate(lua_State *L, DROID_TEMPLATE *template_)
{
	lua_pushinteger(L, template_->multiPlayerID);
}
...
{
	DROID_TEMPLATE *template_;
	
	if (lua_isnumber(L, pos))
	{
		int id = luaL_checkint(L, pos);
		template_ = getTemplateFromMultiPlayerID(id);
		if (template_)
		{
			return template_;
		}
	...
	if (lua_isstring(L, pos))
	{
		...
		template_ = getTemplateFromTranslatedNameNoPlayer(copy);
		if (template_)
		{
			return template_;
		}
...
DROID_GROUP *luaWZObj_checkgroup(lua_State *L, int pos)
{
	...
	if (lua_istable(L, pos))
	{
		...
		group = (DROID_GROUP *)lua_touserdata(L, -1);
		lua_pop(L, 1);
		if (!group)
		{
...
{
	int order = luaL_checkint(L, pos);
	ASSERT_OR_RETURN(DORDER_NONE, order >= DORDER_NONE && order <= DORDER_TEMP_HOLD, "order %d is not a valid droid order", order);
	return (DROID_ORDER)order;
}
