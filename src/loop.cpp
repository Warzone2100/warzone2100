...
static unsigned lastRenderDelta = 0;

void gameAddToLastRenderCost(unsigned int additionalRenderTicks)
{
	if (GetGameMode() != GS_NORMAL)
	{
		return;
	}
	lastRenderDelta += additionalRenderTicks;
}
...
GAMECODE gameLoop()
{
	...
	// adjust render budget based on the time spent on the last frame
	renderBudget += (lastRenderDelta * updateFraction.n);
