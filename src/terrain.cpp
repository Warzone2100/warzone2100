...
#define q(i,j,center) ((x*ySectors+y)*(sectorSize+1)*(sectorSize+1)*2 + ((i)*(sectorSize+1)+(j))*2+(center))
	...
	if (geometryVBO)
		delete geometryVBO;
	geometryVBO = gfx_api::context::get().create_buffer(gfx_api::buffer::usage::vertex_buffer, sizeof(RenderVertex)*geometrySize, gfx_api::context::buffer_storage_hint::dynamic_draw);
	geometryVBO->upload(0, sizeof(RenderVertex)*geometrySize, geometry);
	free(geometry);

	if (geometryIndexVBO)
		delete geometryIndexVBO;
	geometryIndexVBO = gfx_api::context::get().create_buffer(gfx_api::buffer::usage::index_buffer, sizeof(uint32_t)*geometryIndexSize);
	geometryIndexVBO->upload(0, sizeof(GLuint)*geometryIndexSize, geometryIndex);
	free(geometryIndex);

	if (waterVBO)
		delete waterVBO;
	waterVBO = gfx_api::context::get().create_buffer(gfx_api::buffer::usage::vertex_buffer, sizeof(RenderVertex)*waterSize, gfx_api::context::buffer_storage_hint::dynamic_draw);
	waterVBO->upload(0, sizeof(RenderVertex)*waterSize, water);
	free(water);

	if (waterIndexVBO)
		delete waterIndexVBO;
	waterIndexVBO = gfx_api::context::get().create_buffer(gfx_api::buffer::usage::index_buffer, sizeof(uint32_t)*waterIndexSize);
	...
	if (decalVBO)
		delete decalVBO;
	decalVBO = gfx_api::context::get().create_buffer(gfx_api::buffer::usage::vertex_buffer, sizeof(DecalVertex)*decalSize, gfx_api::context::buffer_storage_hint::dynamic_draw);
