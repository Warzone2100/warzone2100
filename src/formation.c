					  ...
					  SDWORD *pX, SDWORD *pY)
{
	...
	xoffset = MAKEINT(FRACTmul(sin(dir),MAKEFRACT(psFormation->rankDist * rank)))
			+ psFormation->asLines[line].xoffset;
	yoffset = MAKEINT(FRACTmul(cos(dir),MAKEFRACT(psFormation->rankDist * rank)))
	...
	*pX = MAKEINT(FRACTmul(sin(dir),MAKEFRACT(dist)))
			+ xoffset + psFormation->x;
	*pY = MAKEINT(FRACTmul(cos(dir),MAKEFRACT(dist)))
