...
static inline void dealWithLMBObject(BASE_OBJECT* psClickedOn)
{
	...
	switch (psClickedOn->type)
	{
		...
		default:
			debug(LOG_ERROR, "Weird selection from LMB - type of clicked object is %d", (int)psClickedOn->type);
			break;
	}
}
...
//					psStructure->selected = true;
		...
		{
			debug(LOG_ERROR, "Weird selection from RMB - type of clicked object is %d", (int)psClickedOn->type);
		}
	}
	else
	{
