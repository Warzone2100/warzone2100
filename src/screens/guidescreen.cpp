...
public:
	...
	bool addTopic(const std::shared_ptr<WzGuideTopic>& topic);
	void clear();
...
bool WzGuideTopicsRegistry::addTopic(const std::shared_ptr<WzGuideTopic>& topic)
{
	return addTopicImpl(topic) != nullptr;
}
...
void WzGameGuideScreen::reopeningScreen(const OnCloseFunc& onCloseFunction)
{
	onCloseFunc = onCloseFunction;
	pauseGameIfNeeded();
}

void WzGameGuideScreen::closeScreen()
{
...
std::shared_ptr<WzGameGuideScreen> createGuideScreen(std::function<void ()> onCloseFunc, bool expandedSidebarByDefault = true)
{
...
std::shared_ptr<WzGameGuideScreen> createOrGetGuideScreen(std::function<void ()> onCloseFunc, bool expandedSidebarByDefault = true)
{
	...
	else
	{
		...
		psCurrentGameGuideScreen->reopeningScreen(onCloseFunc);
	}
...
bool addGuideTopic(const std::shared_ptr<WzGuideTopic>& topic, bool showImmediately, WzGuideForm::OnTopicCloseFunc onTopicClose = nullptr)
{
	std::string guideTopicIdentifier = topic->identifier;
	bool retVal = guideTopicsRegistry->addTopic(topic);

	if (retVal && psCurrentGameGuideScreen)
	{
	...
	if (showImmediately)
	{
		showGuideTopicImmediately(guideTopicIdentifier, onTopicClose);
	}
...
static bool loadWildcardTopicIds(const std::string& guideTopicId, bool showImmediately, WzGuideForm::OnTopicCloseFunc onTopicClose = nullptr)
{
	...
	optional<std::string> firstNewLoadedGuideTopicId = nullopt;
	WZ_PHYSFS_enumerateFilesEx(baseDir.c_str(), [&baseDir, &fullPath, &expectedGuideTopicId, &jsonExtension, &firstNewLoadedGuideTopicId](const char *i) -> bool {
		if (!i) { return true; }
		...
		if (strEndsWith(fullPath, jsonExtension))
		{
			...
			if (addGuideTopic(psNewGuideTopic, false))
			{
				if (!firstNewLoadedGuideTopicId.has_value())
				{
					firstNewLoadedGuideTopicId = psNewGuideTopic->identifier;
				}
			}
		}
	...
	if (showImmediately && firstNewLoadedGuideTopicId.has_value())
	{
		// Just highlight the first new entry added, and show it
		showGuideTopicImmediately(firstNewLoadedGuideTopicId.value(), onTopicClose);
	}
	return true;
}

bool addGuideTopicImpl(const std::string& guideTopicId, bool showImmediately, WzGuideForm::OnTopicCloseFunc onTopicClose = nullptr)
{
	if (loadWildcardTopicIds(guideTopicId, showImmediately, onTopicClose))
	{
		// already handled
		return true;
	}
	...
	return addGuideTopic(loadedTopic, showImmediately, onTopicClose);
}
...
{
	addGuideTopicImpl(guideTopicId, showImmediately, nullptr);
}
