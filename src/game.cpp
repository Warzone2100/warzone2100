...
static void deserializeNetPlay_json(const nlohmann::json &o, NETPLAY *serializeNetPlay)
{
	...
	game.scavengers = o.at("netScavengers").get<bool>();
}
...
static void serializeSaveGameData_json(nlohmann::json &o, const SAVE_GAME *serializeGame)
{
...
static void deserializeSaveGameData_json(const nlohmann::json &o, SAVE_GAME *serializeGame)
{
...
static UDWORD getCampaignV(PHYSFS_file *fileHandle, unsigned int version, nonstd::optional<nlohmann::json> &gamJson)
{
 	...
 	if (version <= CURRENT_VERSION_NUM)
	{
		...
		{
			debug(LOG_INFO, "loading campaign from gam json");
...
bool gameLoadV(PHYSFS_file *fileHandle, unsigned int version, nonstd::optional<nlohmann::json> &gamJson)
{
	...
	else if (version <= CURRENT_VERSION_NUM)
	{
		if (gamJson.has_value())
		{
			debug(LOG_INFO, "gam json found, loading that.");
			deserializeSaveGameData_json(gamJson.value(), &saveGameData);
		} 
		else
		{
			debug(LOG_INFO, "no gam json found, falling back to .gam");
...
static bool writeGameFile(const char *fileName, SDWORD saveType)
{
	...
	char jsonFileName[PATH_MAX] = {0};
	unsigned len = strlen(fileName);
	ASSERT(strcmp(fileName + (len - 4), ".gam") == 0, "hmm... not .gam?");
	memcpy(jsonFileName, fileName, len - 4);
	memcpy(jsonFileName + len - 4,".json", 5);
	// old .gam file
	status = serializeSaveGameData(fileHandle, &saveGame);
	auto gamJson = nlohmann::json::object();
	// new .json format
	serializeSaveGameData_json(gamJson, &saveGame);
	saveJSONToFile(gamJson, jsonFileName);
	debug(LOG_INFO, "saved game into %s", jsonFileName);
...
static nonstd::optional<nlohmann::json> readGamJson(const char* filenameWithGamExtension)
{
	...
	memcpy(jsonFileName, filenameWithGamExtension, len - 4);
	memcpy(jsonFileName + len - 4,".json", 5);
	UDWORD pFileSize;
	char *ppFileData = nullptr;
	debug(LOG_INFO, "starting deserialize %s", jsonFileName);
	if (!loadFile(jsonFileName, &ppFileData, &pFileSize))
	{
		debug(LOG_SAVE, "No %s found, sad", jsonFileName);
		return nullopt;
	}
