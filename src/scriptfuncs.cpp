...
static int scrAddMessage(lua_State *L)
{
	...
	MESSAGE_TYPE msgType = (MESSAGE_TYPE)luaL_checkint(L, 2);
...
static int scrDroidHasTemplate(lua_State *L)
{
	...
	DROID_TEMPLATE* template_ = luaWZObj_checktemplate(L, 2);
	lua_pushboolean(L, strcmp(droid->aName, template_->aName) == 0);
	return 1;
}
