...
//! \brief Retreat positions for the players.
...
//! \brief Initializes RUN_DATA singleton.
...
//! \brief Sends an order with a location target to all selected droids.
extern void orderSelectedLoc(uint32_t player, uint32_t x, uint32_t y, bool add);

//! \brief Sends an order with an object target to all selected droids.
...
//! \brief Sends an order with a location and a stat to all selected droids.
...
//! \brief Sends an order with two a locations and a stat to all selected droids.
